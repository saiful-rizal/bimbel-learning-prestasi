...
export default function PricingSection() {
    return (
        <div id="pricing" className="px-4 md:px-16 lg:px-24 xl:px-32">
            <SectionTitle 
                text1="Biaya Pendidikan" 
                text2="Pilihan Jenjang Pendidikan Siswa" 
                text3="Investasi terbaik untuk pendidikan. Pilih Kategori Siswa yang sesuai dengan jenjang pendidikan putra-putri Anda." 
            />
            ...
            <div className="flex flex-wrap items-center justify-center gap-8 mt-20">
                {pricingData.map((plan: IPricing, index: number) => (
                    ...
                    >
                        {plan.mostPopular && (
                            <p className="absolute px-3 text-sm -top-3.5 left-3.5 py-1 bg-pink-400 rounded-full">Paling Laris</p>
                        )}
                        <p className="font-semibold">{plan.name}</p>
                        <h1 className="text-3xl font-semibold">Rp {plan.price}<span className="text-gray-500 font-normal text-sm">/{plan.period}</span></h1>
